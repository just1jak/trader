...
class BacktestResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            return {'error': _safe_error(exc)}, 502
...
def _data_source_status(probe=False):
    ...
    if probe:
        ...
        sources.append(_source_item(
            ...
            status='ready',
            rows=0,
            detail='Public no-key crypto candles are available. Run a probe for a fresh live check before backtesting.',
            next_steps=['Run Probe sources to validate current request health before collecting a candle set.', 'Collect BTC-USD or ETH-USD candles into the local cache to begin backtesting.'],
        ))
    ...
    if probe:
        ...
        sources.append(_source_item(
            ...
            status='ready',
            rows=0,
            detail='Public stock/ETF OHLCV candles are available. Run a probe for a fresh live check before backtesting.',
            next_steps=['Run Probe sources to validate current request health before collecting a candle set.', 'Collect AAPL or SPY candles into the local cache to begin backtesting.'],
        ))
